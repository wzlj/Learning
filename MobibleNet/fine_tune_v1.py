...
# from tensorflow.contrib.slim.python.slim.nets.resnet_v1 import resnet_v1
...
def train(train_record_file,
    ...
    with slim.arg_scope(mobileNet_v1.mobilenet_v1_arg_scope()):
        out, end_points = mobileNet_v1.mobilenet_v1(inputs=input_images, num_classes=labels_nums,
                                                    dropout_keep_prob=1, is_training=is_training,
                                                    global_pool=True)
...
def fine_tune(train_record_file,
          train_log_step,
          train_param,
          val_record_file,
          val_log_step,
    ...
    with slim.arg_scope(mobileNet_v1.mobilenet_v1_arg_scope()):
        out, end_points = mobileNet_v1.mobilenet_v1(inputs=input_images, num_classes=labels_nums,
                                                    dropout_keep_prob=0.5, is_training=is_training,
                                                    global_pool=True)
    model_vars = tf.trainable_variables()
    slim.model_analyzer.analyze_vars(model_vars, print_info=True)
    ...
    with tf.Session() as sess:
        ...
        path = os.path.dirname(snapshot_prefix)
        best_models = os.path.join(path, 'best_models.ckpt')
        saver.restore(sess, best_models)
...
if __name__ == '__main__':
    ...
    train_log_step = 1
    base_lr = 0.01  # 学习率
    # 重头开始训练的话，mobilenet 收敛慢的一比，大概20000次迭代后，准确率开始蹭蹭的往上长,迭代十万次后准确率才70%
    max_steps = 15000  # 迭代次数
    train_param = [base_lr, max_steps]

    val_log_step = 1
    snapshot = 1  # 保存文件间隔
    # ckpt_path = "models/resnet18_x1/" # x_init = x
    # ckpt_path = "models/resnet18_x2/"  # x_init = conv(x_init)
    ckpt_path = "models/mobileNet_v1/"  # global average pooling is replaced by max_pooling
    if not os.path.exists(ckpt_path):
        os.mkdir(ckpt_path)

    snapshot_prefix = os.path.join(ckpt_path, "model.ckpt")

    fine_tune(train_record_file=train_record_file,

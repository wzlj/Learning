...
# FarmDataset(train_path=args.train_path, label_path=args.label_path, istrain=True)

class FarmDataset(Dataset):
    def __init__(self, train_path=None, label_path=None, istrain=True, isaug=True):
        self.istrain = istrain
        self.trainxformat = []
        self.trainyformat = []
        self.fns = []
        for _path in train_path:
            self.trainxformat.append(os.path.join(_path, '*.png'))
        for _path in label_path:
            self.trainyformat.append(os.path.join(_path, "*.png"))

        print(self.trainxformat)
        print(self.trainyformat)

        for _path in  self.trainxformat:
            print(_path)
            self.fns.extend(glob.glob(_path))
    ...
    def __getitem__(self, idx):
        if self.istrain:
            ...
            _data = imgxname.split("/")[-2]
            _label = "label" + _data[4:]
            # print("\n\n ----------",_data, _label)
            imgyname = imgxname.replace(_data, _label)

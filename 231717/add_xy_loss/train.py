...
from lovasz_losses import lovasz_hinge, lovasz_bce_with_logits, lovasz_focal_symmetric
from focal_losses import binary_focal_loss_with_logits
from dice import dice_loss_with_logits, dice_and_bce
...
def train(args, model, device, train_loader, optimizer, epoch, scheduler, model_path):
    ...
    criterion = functools.partial(eval.criterion, loss_func=loss_func)
    ...
    for batch_idx, (data, target) in enumerate(tqdm(train_loader)):
        ...
        losses = criterion(masks_pred, temp_label)
...
def main():
    ...
    # PATH Setting
    parser.add_argument('--train_path', type=list, default=["../data/train/data1024_0.1"])
    parser.add_argument('--label_path', type=list, default=["../data/train/label1024_0.1"])

    parser.add_argument("--loss", default='lovasz_focal_symmetric', help='loss function of NN',
    ...
    startepoch = 0
    if startepoch:
        model = torch.load('../data/models/model_data1024_0.1/72')
    ...
    model_path = os.path.join('../data/models/model_{}_{}'.format(dataset, time.strftime("%m-%d %H:%M", time.localtime())))

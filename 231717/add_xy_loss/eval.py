...
def mask_to_class_segments(masks, n_class=4):
    for i in range(n_class):
        yield (masks & (2 ** (30-i))) > 0


def mask_to_class_segments_(masks, n_class=4):
    for i in range(n_class):
        yield (masks & (2 ** (30-i))) > 0


def mask_criterion(outputs, targets, loss_func=F.binary_cross_entropy_with_logits):
    ...
    for i in range(c):
        o = outputs[:, i]
        loss = loss_func(o, targets[:, i])
        losses[classnames[i]] = loss
    return losses


def criterion(outputs, targets, loss_func=F.binary_cross_entropy_with_logits):
    return mask_criterion(outputs, targets, loss_func)

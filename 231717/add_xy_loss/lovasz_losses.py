...
from focal_losses import binary_focal_loss_with_logits
...
def lovasz_hinge(logits, labels, **kwargs):
    return lovasz_loss(logits, labels, error_func=hinge, **kwargs)


def lovasz_symmetric(logits, labels, **kwargs):
    return (lovasz_hinge(logits, labels, **kwargs) + lovasz_hinge(-logits, 1 - labels, **kwargs)) / 2


def lovasz_bce_with_logits(logits, labels, **kwargs):
    error_func = functools.partial(F.binary_cross_entropy_with_logits, reduction='none')
    return lovasz_loss(logits, labels, error_func=error_func, **kwargs)


def lovasz_focal(logits, labels, **kwargs):
    error_func = functools.partial(binary_focal_loss_with_logits, reduce=False)
    return lovasz_loss(logits, labels, error_func=error_func, **kwargs)


def lovasz_focal_symmetric(logits, labels, **kwargs):
    return (lovasz_focal(logits, labels, **kwargs) + lovasz_focal(-logits, 1 - labels, **kwargs)) / 2

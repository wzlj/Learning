...
class UNet(nn.Module):
    ...
    def forward(self, x):
        e1 = self.backbone.conv1(x)  # [, 3, 256, 256]
        x1 = self.backbone.bn1(e1)
        x1 = self.backbone.relu(x1)  # [, 64, 128, 128]
        e2 = self.backbone.maxpool(x1)  # [, 64, 64, 64]

        e3 = self.backbone.layer1(e2)  # [, 256, 64, 64]
        e4 = self.backbone.layer2(e3)  # [, 512, 32, 32]
        e5 = self.backbone.layer3(e4)  # [, 1024, 16, 16]
        # x5 = self.backbone.layer4(x4)

        d4 = self.up1(e5, e4)  # 16 --> 32
        d3 = self.up2(d4, e3)  # 32 --> 64
        d2 = self.up3(d3, e2)  # 64 --> 64
        d1 = self.up4(d2, e1)  # 64 --> 128
        u1 = self.up5(d1)  # 128 --> 256

        d1_size = x.size()[2:]
        upsampler = functools.partial(F.interpolate, size=d1_size, mode='bilinear', align_corners=False)
        _d4 = self.conv_1x1_4(d4)
        _d3 = self.conv_1x1_3(d3)
        _d2 = self.conv_1x1_2(d2)
        u4 = upsampler(_d4)
        u3 = upsampler(_d3)
        u2 = upsampler(_d2)

        d = torch.cat((u1, u2, u3, u4), 1)

        x = self.outc(d)
        return torch.sigmoid(x)  # 结尾处没有使用sigmoid

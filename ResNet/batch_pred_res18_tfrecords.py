...
def batch_pred_tfrecords(models_path, record_file, labels_nums, data_format):
    ...
    out = ResNet_.resnet_v1_18(inputs=input_images,
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        images, _ = sess.run([image_batch, label_batch])
        ...
        for _ in range(15):
            start = time.time()
            sess.run([class_id], feed_dict={input_images: images})
            end = time.time()
            print("time of batch {} is %f".format(batch_size) % (end - start))
...
def main(_):
    ...
    batch_pred_tfrecords(models_path, val_record_file, labels_nums, data_format)
